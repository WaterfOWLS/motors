...
class JS_obj:
  ...
  def __init__():
    ...
    try:
      motor_service = rospy.ServiceProxy('motor_power2', MotorPower2)
  ...
  def clamp(max_val, min_val, value):
    return (min(max_val, max(min_val, value)))

  def send_power():
    resp = motor_service(self.pow1, self.pow2)
    print resp
  ...
  def joy_callback(self, data):
    ...
    if data.axes[safety] == safety_threshold:
      self.pow1 = clamp(1, -1, for_back + spin)
      self.pow2 = clamp(1, -1, for_back - spin)
    else:
      self.pow1 = 0
      self.pow2 = 0
